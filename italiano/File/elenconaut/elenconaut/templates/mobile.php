...
<html><head>
 ...
 <meta description="Index of <?= $fullpath ?>" />
 ...
 <h1><?= Elenconaut::breadcrumbs() ?></h1>

 <ul>
 <? foreach ($files as $info): extract($info); ?>
  <li><a href="<?= $link ?>"><img src="<?= $icons . $type ?>.png" title="<?= $type ?>"><?= $name ?></a></li>
 <? endforeach; ?>
 </ul>
 ...
 ( <?= $files ?> file<?= $files === 1 ? '':'s'?>,
   <?= $directories ?> director<?= $directories === 1? 'y' : 'ies' ?> )</p>

 <p id="about">Generated with <a href="http://dreadnaut.altervista.org/progetti/elenconaut/">Elenconaut <?= ELENCONAUT_VERSION ?></a></p>
